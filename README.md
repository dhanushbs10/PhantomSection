...
A C++ shellcode loader utilizing PEB walking, Export Address Table (EAT) parsing, and ETW patching to evade userland API hooking and telemetry. Payloads are staged and XOR-encrypted to bypass static signature analysis.
...
The shellcode is XOR-encrypted (Key: 0x55) and stored separately from the loader as payload.bin. It is read into memory and decrypted only at runtime. This drops the entropy of the compiled executable, bypassing static heuristic and signature-based AV scans.
